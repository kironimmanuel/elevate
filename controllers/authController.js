...
const login = async (req, res) => {
  ...
  const token = user.createJWT()
  // Set password to undefined, so we prevent it from appearing in the response
...
const updateUser = async (req, res) => {
  ...
  // We already have default values for lastName and location
  ...
  // Not necessary, since we are not updating the _id, but nice to have, because it refreshes the token
  const token = user.createJWT()
